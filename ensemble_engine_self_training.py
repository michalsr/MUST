...
def train_one_epoch(model: torch.nn.Module, args, train_config,
    ...
    for step, ((images_weak, images_strong, mask),tokens, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():             
            ...
            l = F.cross_entropy(model_ema.ema(images_weak,tokens),targets)
            metric_logger.update(pseudo_loss=l)
            probs_ema = F.softmax(model_ema.ema(images_weak,tokens),dim=-1)
            # print(probs_ema.size(),'probs ema')
            # for j,p in enumerate(targets):
            #     print(probs_ema[j,p],'targets')
            #print(probs_ema[:,targets],'targets')
            # print(probs_ema,probs_ema.size(),'before')
            if args.pseudo_label_initial:
                    ...
                    var = torch.normal(mean=torch.tensor(.5),std=torch.tensor(.1))
                    #print(var)
                    #print(var,'var')
                    
                    for j,p in enumerate(targets):
                        #print('before',probs_ema[j,p])
                        #print('var',var)
                        probs_ema[j,p] = probs_ema[j,p] + var
                        #print(probs_ema[j,p],'after')
            #     print(probs_ema[j,p],'targets')
                   # probs_ema[:,targets] = probs_ema[:,targets] + var 
                    probs_ema = F.normalize(probs_ema,dim=-1)    
                    #print(probs_ema,'probs') 
            ...
            full_pseudo_label_acc = (pseudo_targets == targets).float().mean().item()
            metric_logger.update(full_training_acc=full_pseudo_label_acc)
        ...
        with amp_autocast():    
            if args.mask:
                ...
                logits = model(images_strong,tokens)
            
            
            # self-training loss    
            if not args.self_train_real_labels:
                loss_st = F.cross_entropy(logits[conf_mask], pseudo_targets[conf_mask])
            else:
                loss_st = F.cross_entropy(logits[conf_mask], targets[conf_mask])
